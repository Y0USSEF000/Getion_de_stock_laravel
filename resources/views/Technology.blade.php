...
<section class="tech-container">
    <h2 style="color: #00ff73; text-align: center;">Produits technologiques disponibles</h2>

    @if($technologies->isEmpty())
        <p style="text-align: center; color: #ccc;">Aucun produit technologique trouvé.</p>
    @else
        <div class="tech-grid">
            @foreach($technologies as $tech)
                <div class="tech-card">
                    @if($tech->image)
                        <img src="{{ asset('storage/' . $tech->image) }}" alt="{{ $tech->name }}">
                    @endif
                    <h3>{{ $tech->name }}</h3>
                    <p>Prix : {{ $tech->price }} MAD</p>
                    <p>Quantité : {{ $tech->quantity }}</p>
                    <p>Email : {{ $tech->email }}</p>
                    <p>Téléphone : {{ $tech->phone }}</p>
